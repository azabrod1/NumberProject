...
public class Arithmetic {
	...
	private final double desiredNum;
	...
	public static void solve(double[] variables, double desiredNum){
		...
		Object[] ans = solver.answer.toArray();
	...
	private double group(int depth){
		...
		while((r & 3) == 0) { // 25% chance
			r = r >> 2;
				if((r & 3) == 0 && index < ttlNums){ //25% chance to raise this group to some power
					answer.add('^');
					result = Math.pow(result, group(depth + 1));
				}
				else if((r & 3) == 1){ //double factorial
					...
					if((result > -0.001 && (Math.abs(result - (int) result)) < 0.0001) && result < 15){ //Result must be integer to do factorial 
